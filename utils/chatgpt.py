...
def chatgpt(words_size):
    ...
    Data:
        ...
        """
        system_text_pt = """
    Esses dados foram extraidos de uma sequencia de telas (screenshots) de um celular (mobile) no seguinte formato: (word, left position, top position, height). Os dados estão em um grande array e cada tela do fluxo em um array dentro. As telas estão em ordem de execução. Identifique o idioma das telas e a sua analise deve ser no idioma que você identificou nas telas. Não precisa me informar o idioma, apenas escreva a sua analise no idioma que você identificou nas telas.
